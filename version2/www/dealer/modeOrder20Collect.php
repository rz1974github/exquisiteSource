...
$_SESSION['oldPoints']=$_POST['oldPoints'];
...
{	
	...
	if(strlen($_SESSION['ship_mobile'])==0)
	{
		if(strlen($errorString)!=0) $errorString.="&";
		$errorString.="mobileError=true";
	}
	
	if(strlen($_SESSION['ship_mobile'])>10)
	{
		if(strlen($errorString)!=0) $errorString.="&";
		$errorString.="mobileTooLong=true";
	}
	...
	{
		header("location:{$_SESSION['pageBase']}?mode=modeOrder&{$errorString}");
		exit(0);
	}//if
...
echo "ship_zip:".$_SESSION['ship_zip']."<br />";
...
{
	header("location:{$_SESSION['pageBase']}?mode=modeOrder3Confirm");
	exit(0);
}
